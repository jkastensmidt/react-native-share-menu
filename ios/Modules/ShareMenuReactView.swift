...
public class ShareMenuReactView: NSObject {
  ...
  func extractDataFromContext(
    context: NSExtensionContext, withCallback callback: @escaping ([Any]?, NSException?) -> Void
  ) {
      ...
      for item in items {
        ...
        guard let attachments = item.attachments else {
          callback(
            nil,
            NSException(
              name: NSExceptionName(rawValue: "Error"), reason: "couldn't find attachments",
              userInfo: nil))
          return
        }
        ...
        for provider in attachments {
          if provider.hasItemConformingToTypeIdentifier(kUTTypeURL as String) {
                ...
                if image != nil {
                  ...
                  guard
                    let imageURL = NSURL(fileURLWithPath: NSTemporaryDirectory())
                      .appendingPathComponent("TemporaryScreenshot.png")
                  else {
                    return
                  }
                  ...
                  do {
                    ...
                    results.append([
                      DATA_KEY: imageURL.absoluteString, MIME_TYPE_KEY: imageURL.extractMimeType(),
                    ])
                  } catch {
                    callback(
                      nil,
                      NSException(
                        name: NSExceptionName(rawValue: "Error"), reason: "Can't load image",
                        userInfo: nil))
                  }
                }
              }
            ...
            provider.loadItem(forTypeIdentifier: kUTTypeData as String, options: nil) {
              (item, error) in
              let url: URL! = item as? URL

              results.append([
                DATA_KEY: url.absoluteString, MIME_TYPE_KEY: self.extractMimeType(from: url),
              ])
              semaphore.signal()
            }
            semaphore.wait()
